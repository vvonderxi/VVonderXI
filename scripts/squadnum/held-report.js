...
const by = {};
held.forEach((h) => { (by[h.reason] = by[h.reason] || { cs: 0, cards: 0 }).cs++; by[h.reason].cards += h.cards; });
...
console.log('HELD SET , ' + done.length + ' of 1,018 club-seasons attempted\n');
...
console.log('\n  TOTAL ' + held.length + ' club-seasons, ' + held.reduce((a, b) => a + b.cards, 0) + ' cards held');
...
const gap = done.length - new Set(held.map((h) => h.key)).size - produced.size;
console.log('  reconciliation: held ' + new Set(held.map((h) => h.key)).size + ' + produced ' + produced.size +
            ' = ' + done.length + (gap === 0 ? '   RECONCILES' : '   GAP OF ' + gap + ' , investigate'));
