...
- **SAME-LEAGUE MID-SEASON TRANSFERS ARE STORED AS ONE HALF ON ROUGHLY TWO THIRDS OF SPLITS, AND THE CAUSE IS STALE DATA, NOT THE CURRENT RESOLVER (measured 2026-08-23, NOT repaired).** Measured against the provider across three league-seasons: **34 of 51 genuine splits are halved , PL 2023/24 7 of 12, PL 2025/26 9 of 13, SA 2023/24 18 of 26.** Worst: **Semenyo 1,798 of 3,200 minutes, Guehi 1,800 of 3,150, B. Johnson 628 of 1,752.** **THE POPULATION IS 1,462, MEASURED AGAINST THE PROVIDER ON 2026-09-04** (`docs/HANDOVER_2026-09-04.md`), of which **968 are material repairs , 461 RAISE the per-90 rate and 287 LOWER it.** The earlier extrapolation of ~1,600 from three league-seasons is SUPERSEDED and has been removed wherever it stood, because a figure with no source gets quoted rather than re-derived. **FROM STORED DATA THE POPULATION IS NOT MEASURABLE AT ALL** , see the halved-card entry in `CLAUDE.md` SS E: the only internal detector is `player_positions.appearances > card.appearances`, it returns **841**, and it cannot see the 20,381 cards (36%) that carry no pp row or a NULL one. **Only a provider scan can size this, which is an argument for the re-import beyond repair.**
  ...
  - **REPAIR IS rt-TOUCHING AT SCALE AND IS SCOPED BUT NOT RUN , see the plan in `INGESTION_RECOVERY.md`.** `--insert-only` CANNOT fix a halved row (ON CONFLICT DO NOTHING), and a default-mode re-run rewrites all ~57,000 rows rather than the 1,462 that are wrong.
