...
Stage 2 ships on the FLAT PROVISIONAL league ladder (PL 1.00, Portugal 0.845, etc.) , the placeholder weights, NOT computed. Decision 3 (endogenous dynamic per-season league strength) is DESIGNED but NOT BUILT. So ALL of Stage 2's league-scaling , the TAA/Grimaldo separation, the wt^3.5 boost, the tilt , currently runs on provisional weights. **STAGE 3 = build dynamic league strength (the original launch-blocker).** When it lands, computed weights flow through the SAME `wt^3.5` socket + the tilt socket, and Stage 2's exact numbers will REFINE (TAA/Grimaldo/all league-scaled figures move). The method is refinable without touching the Stage-2 pipe , only the weight table swaps. Migration: migrations/stage2_bounded_def_bestof.sql. **>>> SUPERSEDED 2026-07-10: Stage 3 BUILT + SHIPPED, weights are now COMPUTED (DECISION 7). <<<**

---

## DECISION 7 , STAGE 3 SHIPPED: ENDOGENOUS LEAGUE STRENGTH (computed weights replace placeholders) (LOCKED + SHIPPED 2026-07-10)

Decision 3 built + live. Placeholder league ladder RETIRED , league weights now COMPUTED from our own data.

**METHOD (circularity-safe, rt-blind):** league_strength(league, season) from 980 age-adjusted MOVER observations (>=900 min each side of an adjacent-season cross-league move, >=3 G+A). obs(A->B) = ageCurve[age] - dlog(output), solved as weighted least-squares over the 9-league network anchored PL=1.00, medians per edge (outlier-robust), rolling +-2 season window. Age curve from ~21.7k stayer pairs. Reads ONLY raw goals+assists per 90 + league scoring rate. Scripts: scripts/engine/league_strength.js (v1) + league_strength_v2.js (env-corrected + loader). Weights table: engine_league_weights (144 rows, 9 leagues x 2010-2025), engine_league_weights.csv.

**SCORING-ENVIRONMENT CORRECTION (alpha=0.5, the key fix):** raw ga90 read high-scoring leagues (Bundesliga, Eredivisie) as WEAK (their open style inflates output, so movers looked like they left an easy league , BL landed 0.67-0.75, BELOW Ligue 1). FIX: normalize each player-season output by its league's own scoring rate: rel = ga90 / env(league,season)^alpha, env = league aggregate (g+a)/90. alpha=0.5 chosen after inspecting raw/0.5/0.65/1.0: full (a1.0) over-corrected (BL above La Liga); a0.65 reshuffled the top (BL>LL) WITHOUT fixing the residuals; **a0.5 lifts BL into the top-4 cluster, keeps PL/LL/SA cleanly on top, weak leagues low.** DIAGNOSTIC LOCK: the two residual soft spots (BL~L1 in the 2020 window, TR~L1 in 2016) do NOT move with alpha -> they are genuine mover-data features of those eras, NOT the style confound, so chasing them would fabricate. Disclosed, not engineered.

**SOCKET RE-TUNE (required for the spread weights):** placeholder ladder was compressed (0.76-0.98); computed is spread (0.51-1.0). The tilt `(1-(1-wt)*0.5)` + boost `wt^3.5` were calibrated for the compressed range, so feeding spread weights through them OVER-swung (weak-league stars dropped -13). Re-tuned to fit the range the sockets now carry: **tilt coefficient 0.5->0.35, boost exponent 3.5->2.5.** This fits the sockets to the weights, does NOT distort the honest ladder. Result: whole-league-star drops fell from -13 to ~-5/-7; PL unaffected (at wt=1 the tilt is identity).

**SHIP DIFF (gated, approved before live):** 67% of cards move, mean -1.14. Per-league: PRT -3.1, ERE -2.1, BL -1.7, LL -1.3, L1 -1.1, BPL -1.0, SA -0.4, PL 0, TR 0. Read-outs HELD: Messi 97, Ronaldo 96 unchanged; Haaland 94->95, TAA 85->86 (PL relative-rise as the non-PL field settles below); van Dijk 74. Weak-league: Grimaldo Benfica'22 78->71 (-7); Portuguese 20-goal strikers ~86->80; ELITE weak-league scorers barely move (Gyokeres 39g PRT 93->91) , pre-existing output-first behavior (a 39-goal season is elite wherever scored), ACCEPTED, not a Stage-3 artifact. Bundesliga modest (Lewandowski 95->93). League separation intact (Grimaldo BL > PRT at equal output). Verified 0 rt mismatches vs candidate before matview refresh. Migration: migrations/stage3_league_strength.sql; rollback: migrations/stage3_ROLLBACK_prev_view.sql; new baseline: engine_baseline_stage3.
...
### STAGE 4 EXECUTED (2026-07-14) , 61 clear attackers reclassified out of CDM, matview live
- **Flagging reproduced this scope exactly:** CDM regulars 2152, output>=Q3 47. 121 flagged total = 81 coarse=FWD + 40 output-only.
- **SCOPE SHIPPED = "clear attackers only" (Lucas-approved Option 1):** reclassified 61 rows / 45 players (50->Winger, 9->ST, 2->CAM) , the coarse=FWD clear wide-attackers/forwards WITH in-DB attacking corroboration (an attacking-pool season elsewhere OR real ga90). Destinations HAND-VERIFIED by archetype, NOT the noisy per-season modal (the API per-season pool labels are pure noise , Rodri himself shows Winger/CAM some seasons , so "modal attacking pool" wrongly sent #10s to ST; hand-assigned instead).
- **DELIBERATELY EXCLUDED (kept CDM):** (a) all 40 Bucket-C output-only rows = deep-playmakers/registas the API itself calls midfielders (Eriksen Spurs, Pjanić, Veerman, Fàbregas-deep, Banega, Sneijder, **Çalhanoğlu@Inter**). The output signal cannot separate a mislabeled #10 from a genuine regista; leaving them CDM is defensible and HOLDS the CLAUDE.md Çalhanoğlu@Inter->CDM lock (master doc wins over this log's line-166 "Calhanoglu tagged CDM = mislabel"). (b) coarse=FWD false-positives = central/def mids mis-tagged FWD (Jobe Bellingham 0g/ga90 0.05, Amiri, Jakić, Aursnes, Gerhardt CM, Sergio Gómez FB). (c) 5 obscure single-CDM-season names with NO in-DB corroboration (Susaeta/Torje/Jantscher/D.Larsson/Lazarević) , principle: don't assert what the data can't support.
- **###CORRECTION to the line-169 "~1-2" estimate### the real inflator was the def_core FLOOR, not the output-rarity boost.** Actual: mean rt delta **-5.3, range 0 to -31, 0 cards inflated**. The CDM def_core FLOOR (44 + 22*def_signal, ~44-51) was propping ZERO-output wingers at a defender's competence baseline. Reclassifying to an attacking pool (no floor, output-only) drops non-producers to honest value (Usami 0g/436min 51->20; Frei 0g 47->22) while REAL scorers are UNCHANGED (Benavente 9g, Pedro Henrique 9g = delta 0). So the mislabel was inflating these cards MORE than this doc estimated; the fix is correspondingly more valuable. Peaks/genuine-CDMs verified untouched (Rodri 86, Çalhanoğlu 85/85/81, Pjanić 81, Veerman 81).
- **###OPEN QUESTION (disclosed choice, not oversight)### the same def_core floor still props the ~40 Bucket-C deep-playmakers left in CDM** (Eriksen's Spurs 88/89 seasons etc.). Consistent with the Çalhanoğlu lock + goals-primacy (registas are genuine deep players), so NOT reclassified. Revisit ONLY if deep-playmaker treatment is reopened , would need a regista-vs-mislabel discriminator the aggregate data currently lacks (same invisibility as the elite-CB-class problem, Decision A).
- Pipeline (scripts/enrichment/, dry-run-first): cdm_flag.js -> cdm_adjudicate.js -> cdm_build_plan.js (-> cdm_reclass_plan.csv + cdm_dropped.csv) -> cdm_write.js --write (guarded UPDATE, rt baseline cdm_rt_before.csv) -> cdm_verify.js. Rollback = re-UPDATE the 61 back to CDM from the baseline CSV.

**ENGINE TRACK STATUS: Stages 0-4 DONE + LIVE, verified internally consistent. Engine recalibration COMPLETE. Remaining = optional/post-launch (dribbles_past re-ingest, radar percentile, GK saves). Center of gravity returns to the launch/Compare track.**
