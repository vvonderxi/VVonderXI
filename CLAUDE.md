...
Compare        ███████████████░░░  ~88%   spine LIVE + verified (verdict/accolades/radar/trajectory/deep-link/back-path all working); #101 = FLOW-POLISH not feature-build; real gap = go-to-player link out; + user-controlled fold, back-QA, subheading clip, C8 verify, merge remain
...
2. **COMPARE #101 , FLOW-POLISH pass, NOT a feature-build (spine is BUILT + LIVE, verified live 2026-07-16).** DONE + working (do NOT re-build): shared token-AND search (byte-identical to rankings, same player_card_mv path); verdict "The Edge" (rt decides -> exact tie breaks to AI via /api/analyse, Drury/Winter two-register voice, server-side verdict_cache by card-pair, free-for-all no auth); accolades (real honours , incl. Silverware post-#2 team-join , + profile tags via renderWonderTagsGrouped, both slots); radar (drawDualRadar from live CMP_A/B.radar); trajectory; deep-link ?a=&b=; smart back-path vvBack() wired to a visible .vvback control. The old "kill hardcoded Henry/Haaland" framing was STALE , they are demo objects nulled at compare.html:819; the spine was wired in the Batch-B sessions. REMAINING = flow polish only: (a) **GO-TO-PLAYER , the ONE genuine gap:** no link OUT from a filled slot/verdict to that player's card.html (deep-link IN works, no way OUT); (b) user-controlled fold (per-slot season fold auto-collapses on season-click); (c) back-path cold-deep-link QA (entry with no in-app history) + subheading text clip (minor CSS); (d) picker result-cap "show more"/count (optional, picker caps at 50); (e) C8 position filter , picker ALREADY has working pos chips (pkPosGroup), verify if this is already satisfied. Radar percentile-within-position scaling stays PARKED on the engine track (provisional RADAR_REF placeholders , a known engine caveat, NOT a Compare bug). Then final polish + merge.
