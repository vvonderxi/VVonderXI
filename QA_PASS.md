...
- **3. A VOID IS NOT A PASS AND MUST NEVER BE COUNTED AS ONE.** Two elements are currently unmeasurable and both are recorded with their reason rather than quietly dropped:
  ...
  - **`span.lgco` , "no ink mass above the absolute floor", both themes.** 11.5px at `opacity:.55`, and the crop is too small and too faint for any cluster to clear the 25-device-pixel floor. **What it needs is a larger capture scale** , the floor is absolute in DEVICE pixels, so capturing at scale 3 or 4 instead of the display's 1-2 would put the same glyphs over the floor. That is a one-parameter change and it is NOT made here, because raising the scale changes every reading on the page and would need the control re-run against it. **Until then `.lgco` is UNMEASURED, and it carries the same `opacity:.55` shape as the `.lgfoot` that measured 1.19.**
