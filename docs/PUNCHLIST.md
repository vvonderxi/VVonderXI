...
| 37 | Halved cards , the second half of a same-league mid-season move is refused | **SCOPED FOR SPLIT, AWAITING LUCAS** | Lucas | `docs/HALVED_CARDS_SCOPE.md`. ONE CARD PER CLUB, decided 2026-09-21. **1,740 candidates, ~two thirds real.** Split re-measured: 73 existing cards move, ZERO band crossings, the originals do not move , against summing's 201 and 12. `team_id` is ALREADY on the table and `def_share` is view-derived, so the constraint is the whole schema change. 5 of 19 halves fall under the 300-minute floor and arrive NR |
