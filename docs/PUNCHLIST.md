...
| 26 | Transfer shirt-number confidence flag | **BUILT, AWAITING LUCAS** | Lucas | Mark H, under the shield, gap >= 3, 2016+, **477 cards**. Opt-in on the card hero and the card-page share only. Tap opens two conditional notes in Data Confidence. Rendered: tap 0 flips / 0 season steps, 32px hit area, share PNG carries it. **Precision at gap >= 3 is ~70% by proxy, which is why every line says "if"** |
...
| 37 | Halved cards , the second half of a same-league mid-season move is refused | **SCOPED FOR SPLIT, AWAITING LUCAS** | Lucas | `docs/HALVED_CARDS_SCOPE.md`. ONE CARD PER CLUB, decided 2026-09-21. **1,740 candidates, ~two thirds real.** Split re-measured: 73 existing cards move, ZERO band crossings, the originals do not move , against summing's 201 and 12. `team_id` is ALREADY on the table and `def_share` is view-derived, so the constraint is the whole schema change. 5 of 19 halves fall under the 300-minute floor and arrive NR . **Build plan: `docs/HALVED_SPLIT_BUILD_PLAN.md`** , own sitting, before-snapshot, rollback, two guards. Individual honours COUNTED: exactly ONE (Harbaoui, Golden Boot, BPL 2017), flag goes on the half with more minutes and MOVES off today's card by 11 minutes |
