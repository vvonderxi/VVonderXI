-- player_card_view , DEFINITION AS IT STOOD BEFORE THE 2026-09-04 HONOURS + STAGE APPEND
-- Captured 2026-09-04 from pg_get_viewdef('public.player_card_view', true), verbatim.
-- Body length asserted at 11935 chars on capture and again on write.
--
-- THIS FILE IS THE ROLLBACK. To revert the append, run this body under
--   CREATE OR REPLACE VIEW public.player_card_view AS <body>;
-- It restores the 65-column shape. The matview was never touched, so no refresh is needed
-- to roll back; player_card_mv still selects its own frozen 60-column list.
--
 WITH scored AS (
