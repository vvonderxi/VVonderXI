...
- **`vv-core.js` CACHE TOKEN IS MANUAL , BUMP `?v=` ON EVERY SHIPPING SURFACE THAT LOADS IT WHENEVER vv-core.js CHANGES, OR CLIENTS GET THE STALE FILE. GREP FOR THE SURFACES, DO NOT COUNT THEM.** Added 2026-08-11 (`?v=20260811a`), **CURRENT VALUE `?v=20260823b` as of 2026-08-23** , the token has been live on all three pages since it was added, so any note calling the cache-buster outstanding is STALE (it was still listed as pending in the 2026-08-09 handover). **[THAT PARENTHESIS IS NOW FALSE: playbook and index BOTH load it, since 2026-08-28 and 2026-08-29. GREP, DO NOT COUNT , `grep -l 'vv-core.js\|vv-marks.js' *.html`.]** **The token does not maintain itself: there is no build step, so nothing bumps it for you.** Forgetting is the SAME failure the token exists to fix , a stale cached copy makes any vv-core change **silently no-op while looking fine**, which cost real debugging time when a new export came back `undefined` and again when cached copies served the OLD tag logic after `c4ad9e2`. **HARD-REFRESH after any vv-core change**, and suspect this first when a shared-renderer edit "does nothing".
...
- **`vv-core.js` CACHE TOKEN IS MANUAL , BUMP `?v=` ON EVERY SHIPPING SURFACE THAT LOADS IT WHENEVER vv-core.js CHANGES, OR CLIENTS GET THE STALE FILE. GREP FOR THE SURFACES, DO NOT COUNT THEM.**  **, the incident history is in `RULE_EVIDENCE.md`** **GREP, DO NOT COUNT: the surface list is `grep -l 'vv-core.js\|vv-marks.js' *.html`, and the number written in any rule is a snapshot that has already been wrong three times (three pages, then a fourth at playbook, then a fifth at index).** **The token does not maintain itself: there is no build step, so nothing bumps it for you.** Forgetting is the SAME failure the token exists to fix , a stale cached copy makes any vv-core change **silently no-op while looking fine**. **HARD-REFRESH after any vv-core change**, and suspect this first when a shared-renderer edit "does nothing".
...
- **THE POST-REFRESH DRIFT OF THE TWO EMBEDDED SNAPSHOTS, MEASURED (2026-09-12, after the `player_card_mv` refresh of the same day).** Both had been stale since that refresh, held on instruction and released here. **This is the measured diff, not the pre-refresh simulation** , the simulation predicted fourth-decimal median movement and was the right order of magnitude, but a prediction is not a measurement and the two must never be quoted interchangeably.
  - **`RADAR_POOL_REF`, regenerated by `scripts/gen-radar-ref.js`** , 57,055 rows read, 25,901 into the reference at the unchanged 900-minute gate. **166 of 840 breakpoints moved (19.8%), and not one moved by more than 0.013 in absolute terms or 0.95% of its own value.**

        pool     breakpoints moved   largest move   where
        CB                  0 / 84         0.0000   ,
        FB                 17 / 84        +0.0020   creation p95
        CDM                27 / 84        -0.0130   creation p95
        CM                 27 / 84        +0.0040   creation p90
        CAM                40 / 84        +0.0070   defensive p80
        Winger             40 / 84        -0.0050   creation p90
        ST                 15 / 84        -0.0040   progression p95
        DEF / MID / FWD     0 / 84 each    0.0000   ,

  - **WHAT THAT DOES TO RENDERED RADARS IS THE FIGURE THAT MATTERS, AND IT IS SMALL: 1,641 of 52,766 move, 3.11%, and EVERY SINGLE MOVE IS EXACTLY ONE POINT on a 0 to 100 axis.** Largest in the database is **Salah 2025 progression 32 -> 31**. By pool: CAM 3.45%, Winger 2.98%, CDM 2.24%, CM 2.08%, FB 0.94%, ST 0.91%, and **CB and the three coarse pools do not move at all.** Measured by scoring every card through the shipped `pct()` against both tables, not by reasoning from the breakpoints.
  - **THE MARGIN TABLE (`vv-margin.js`), via `pull_inputs.js` -> `bootstrap_se.js SAMPLE=all REPS=200 MODEL=pois` -> `gen_margin_table.js`: 396 of 50,269 card SEs moved, 0.79%**, 388 of them by a single 0.2 quantisation step and the largest by 1.6. **Median SE 5.80, p95 12.60, max 17.80, identical before and after**, same 50,269 card ids in both tables with none added or dropped.
  - **THOSE 396 ARE DRIFT AND NOT MONTE CARLO NOISE, AND THAT HAD TO BE CHECKED BEFORE IT COULD BE CLAIMED.** `bootstrap_se.js` uses a seeded mulberry32 (`seed 20260909`, a constant), and the card set is byte-identical between runs, so the replicate stream aligns and a re-run on unchanged inputs is deterministic. **Without that check the whole diff would have been unattributable** , a bootstrap with an unseeded RNG moves a few hundred quantised SEs on every run by construction.
  - **AND THE DECISION-LEVEL ANSWER, WHICH IS WHAT THE TABLE EXISTS FOR: ZERO CROWN DECISIONS CHANGE.** Of the 110 rows in `verdict_cache`, 100 resolve to two scored outfield cards; for all 100, `gap > marginFor(a,b)` returns the same verdict under the old and new tables. **A snapshot can be stale and still be harmless, and the only way to know which is to test the decision rather than the number.**
  - **THE ENGINE REIMPLEMENTATION STILL VALIDATES AGAINST THE REFRESHED DATA, so the drift is in the data and not in the transcription: 99.57% exact against stored rt (recorded expectation 99.56%), 99.98% within 1, anchors landing independently on b95 119.91, b90 98.89, b85 85.58, and the fast anchor-swap control at 0 mismatches on 3,000 trials.** Running the controls before the bootstrap is the reason the 396 can be read as drift at all.
  - **THE PROCEDURAL TRAP THIS RUN EXPOSED IS IN `SILENT_FAILURES.md`** , `gen_margin_table.js` wrote to a bare relative path and put the table in the script directory while printing "wrote vv-margin.js". Fixed in the generator in the same pass.


---

- **THE CAREER-ARC SPAN FOR "THE STANDARD" , WHAT THE SCHEMATIC CLAIMED, AND WHAT THE CAREERS SAY (measured 2026-09-12, all 58 holders).**
  - **READ THIS SENTENCE BEFORE ACTING ON ANYTHING BELOW. A HARDCODED COORDINATE IS THE CONVENTION THIS WHOLE DIAGRAM RUNS ON, NOT A DEFECT, AND THE OTHER FOUR MARKERS ARE CORRECT EXACTLY AS THEY ARE.** The playbook career arc is an EXPLAINER DIAGRAM with no player in it: the curve is a fixed SVG path and Wonderkid `3.24%`, Breakout `23.0%`, Peak `50.0%` and The Last Dance `96.76%` are inline literals, with nothing repositioning them at runtime. **This entry is not licence to rip them out.** Each of those four is ONE card per player by construction, so a placed point is the honest shape. **The Standard was the only one whose subject is an EXTENT**, which is why it alone could not be drawn as a placed point, and why only it changed.
  - **THE DEFINITION WAS NEVER AMBIGUOUS AND WAS NOT REDEFINED.** Live SQL, from a fresh `pg_get_viewdef`: `stage_the_standard = COALESCE(nc.n_all >= 2 AND sa.n80 >= 5 AND sb.rt >= 80, false)`, where `n80` counts that player's seasons at rt >= 80 and `sb.rt` is the card's own. The JS reference `careerStageTags` computes the same thing (`rts.filter(v => v >= 80).length >= 5 && row.rt >= 80`), and the shipped copy already describes it correctly: *"Elite not once but for years. Five seasons at the top level."* **413 cards across 58 players. Only the RENDERING was wrong.**
  - **WHAT THE OLD BAR CLAIMED, AND THE MEASUREMENT AGAINST IT.** `left:23%; right:0` with a gradient fading from 72% reads as three statements: the run starts at Breakout, it reaches the end of the career, it tails off.

        first qualifying season, position in career   p10 0.0%   median 11.1%   p90 33.3%
        last  qualifying season, position in career   p10 66.7%  median 92.9%   p90 100.0%

    **44 of 58 holders first qualify BEFORE the 23% start. 32 of 58 stop before the end; only 26 of 58 reach it. The 72% fade encodes nothing at all.**
  - **AND THE QUALIFYING SEASONS ARE OFTEN NOT CONTIGUOUS, so no solid bar of any width can describe them.** Firmino qualifies in 2013, **not** 2014, then 2015 through 2020, **not** 2021, then 2022. A span is an envelope, not a run.
  - **ZERO-WIDTH IS STRUCTURALLY IMPOSSIBLE , DO NOT ADD A DEGENERATE-CASE BRANCH FOR IT.** The rule needs five qualifying cards, and a first-to-last span over five points is **at least four index steps**, so the minimum span fraction is `4/(n-1)`. On the longest career in the data (n = 18) that is **23.5%**; on a hypothetical 25-season career it is still **16.7%**. Measured across the 58: minimum index steps **4**, minimum distinct qualifying years **5**, narrowest holder Eriksen at 23.5%. **A defensive branch for a single-season or adjacent-season span is dead code by construction.**
  - **THE SHIPPED FIX REMOVES THE COORDINATES RATHER THAN CORRECTING THEM** , full width, hatched, no start, no end, no fade, label below the bar and left-aligned. The bar now makes exactly one claim, which is the definition: this persists across a career while the four markers above are moments. **A per-player version that encodes the real extent is a genuinely better idea and is a NEW FEATURE on a surface that has a player , spec parked in `POST_LAUNCH.md`.**
  - **ONE MEASUREMENT THAT KILLED THE INLINE LABEL, worth keeping because it is not obvious:** at 390 the stage track is 318px and the narrowest real span renders a **47px** bar, while the marker plus gap plus label needs **121px**. The label overflowed the bar's right edge by **74px** and read as part of the extent , reintroducing the false precision in a new place. Below the bar it cannot.
