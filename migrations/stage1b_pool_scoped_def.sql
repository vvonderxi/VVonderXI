-- Stage 1b (engine recalibration) , RE-SCOPE the defensive inspection percentiles to position_pool
-- (CB/FB/... , the design's real pool), not coarse psc.position (DEF), AND expose duel quality.
-- Still INSPECTION only, rt UNTOUCHED (scored/ref/ranked/base/anchors/vv identical to live).
-- Changes vs Stage 1:
--   * team_def unchanged.
--   * dshare/dshare_pct replaced by pool_ingr/pool_pct: joins players+player_positions to get the
--     8-bucket position_pool, and percentiles def_share / def90 / duel_rate WITHIN that pool, 2016+.
--   * def_share_pct redefined (now pool-scoped, same name/type/order , create-or-replace allows this).
--   * 3 new inspection columns appended: def90_pool_pct, duel_rate, duel_quality_pct.
--   * def90 (raw) + defvol_pct (OLD coarse rank) kept as-is for continuity.
-- Percentile caveat (mirrors the existing engine): null-valued rows sort last and remain in the
-- partition denominator, so a pool with many null duel_rate/def_share rows slightly compresses the
-- non-null percentiles. Minor for regular CB/FB seasons; disclosed, not corrected, at inspection stage.
 ...
 WITH scored AS (
        ...
        pool_ingr AS (
         SELECT r.card_id,
          ...
          WHERE r.pos <> 'GK'::text AND r.season_year >= 2016 AND pp."position" IS NOT NULL
        ),
        pool_pct AS (
