-- Stage 2 SHIP (DECISION 6) , FIRST live rt change. Bounded def_core + best-of + league-scaled
-- defender output boost. Full live view: keeps all Stage-1b columns (47 base + 8 inspection) and
-- ONLY changes the rt path (base CTE). Params: def_core CB/FB/CDM 44/22/64, CM 32/24/60,
-- ST/Winger/CAM 12*sig; output boost LEAST(12, 0.45*gaw*wt^3.5) on defensive pools.
-- NOTE: runs on PLACEHOLDER (flat) league weights , Stage 3 (dynamic league strength) swaps them
-- into the same wt^3.5 + tilt sockets. Rollback: migrations/stage2_ROLLBACK_prev_view.sql.
         ...
         SELECT r.card_id,
            (0.70::double precision * GREATEST(
                (0.65::double precision * ((0.50::double precision * (0.60::double precision * r.pos_pct + 0.40::double precision * COALESCE(r.abs_pct, 0::double precision)) + 0.50::double precision * (0.60::double precision * r.posvol_pct + 0.40::double precision * COALESCE(r.absvol_pct, 0::double precision))) * 100::double precision) + 0.35::double precision * ((100::numeric * r.gaw)::double precision / NULLIF(rf.gaw_ref, 0::double precision))
                 + CASE WHEN ps.pool IN ('CB','FB','CDM') THEN LEAST(12::double precision, 0.45::double precision * r.gaw::double precision * power(r.wt, 3.5::numeric)::double precision) ELSE 0::double precision END),
                COALESCE(CASE
