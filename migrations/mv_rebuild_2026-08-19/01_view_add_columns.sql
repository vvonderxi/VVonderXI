-- STEP 1 , extend player_card_view with the 10 raw columns (Groups A + B).
-- NO OUTAGE: the site reads the MATVIEW, never this view.
--
-- Generated from a FRESH pg_get_viewdef on 2026-08-19 (11,696 chars, read in chunks and
-- asserted against length() before use), per the CLAUDE.md rule never to rebuild a view
-- from the repo copy. The ONLY difference from the live body is a comma after
-- pool_pct.duel_quality_pct and the 10 appended psc.* lines , the engine is untouched.
--
-- CREATE OR REPLACE VIEW can only APPEND columns, which is why they go at the END, after
-- the eight defensive columns. Changing the order of the existing 55 would fail loudly.
