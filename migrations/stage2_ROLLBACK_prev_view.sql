-- ROLLBACK , EXACT live player_card_view definition captured immediately BEFORE Stage 2 ship (2026-07-10).
-- To revert: run this whole statement in Supabase SQL editor (or via exec_sql), then REFRESH MATERIALIZED VIEW player_card_mv.
CREATE OR REPLACE VIEW player_card_view AS
 WITH scored AS (
         ...
         SELECT r.card_id,
          ...
          WHERE r.pos <> 'GK'::text AND r.season_year >= 2016 AND pp_1."position" IS NOT NULL
        ), pool_pct AS (
