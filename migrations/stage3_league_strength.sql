-- Stage 3 SHIP , endogenous league strength (Decision 3/7). Swaps flat placeholder league weights for
-- computed alpha=0.5 per-season weights (engine_league_weights, scoring-env-corrected) + re-tunes sockets
-- for the spread range: tilt 0.5->0.35, boost exponent 3.5->2.5. Prereq: engine_league_weights loaded
-- (scripts/engine/league_strength_v2.js --load). Rollback: migrations/stage3_ROLLBACK_prev_view.sql.
CREATE OR REPLACE VIEW player_card_view AS
 WITH scored AS (
         ...
         SELECT percentile_cont(0.99::double precision) WITHIN GROUP (ORDER BY (scored.gaw::double precision)) AS gaw_ref FROM scored WHERE scored.pos <> 'GK'::text
        ), ranked AS (
         ...
         SELECT pool_ingr.card_id, pool_ingr.pool, pool_ingr.team_def90, pool_ingr.def_share, pool_ingr.duel_rate, pool_ingr.def90,
            CASE WHEN pool_ingr.def_share IS NOT NULL THEN percent_rank() OVER (PARTITION BY pool_ingr.pool ORDER BY pool_ingr.def_share) ELSE NULL::double precision END AS def_share_pct,
            CASE WHEN pool_ingr.def90 IS NOT NULL THEN percent_rank() OVER (PARTITION BY pool_ingr.pool ORDER BY pool_ingr.def90) ELSE NULL::double precision END AS def90_pool_pct,
            CASE WHEN pool_ingr.duel_rate IS NOT NULL THEN percent_rank() OVER (PARTITION BY pool_ingr.pool ORDER BY pool_ingr.duel_rate) ELSE NULL::double precision END AS duel_quality_pct
           FROM pool_ingr
        ), pool_sig AS (
         ...
         SELECT r.card_id,
            (0.70::double precision * GREATEST(
                (0.65::double precision * ((0.50::double precision * (0.60::double precision * r.pos_pct + 0.40::double precision * COALESCE(r.abs_pct, 0::double precision)) + 0.50::double precision * (0.60::double precision * r.posvol_pct + 0.40::double precision * COALESCE(r.absvol_pct, 0::double precision))) * 100::double precision) + 0.35::double precision * ((100::numeric * r.gaw)::double precision / NULLIF(rf.gaw_ref, 0::double precision))
                 + CASE WHEN ps.pool IN ('CB','FB','CDM') THEN LEAST(12::double precision, 0.45::double precision * r.gaw::double precision * power(r.wt, 2.5::numeric)::double precision) ELSE 0::double precision END),
                COALESCE(CASE
                    WHEN ps.pool IN ('CB','FB','CDM') THEN LEAST(64::double precision, 44::double precision + 22::double precision * ps.sig)
                    WHEN ps.pool = 'CM' THEN LEAST(60::double precision, 32::double precision + 24::double precision * ps.sig)
                    WHEN ps.pool IN ('ST','Winger','CAM') THEN 12::double precision * ps.sig
                    ELSE NULL::double precision END, 0::double precision)
              ) + (0.30 * LEAST(95::numeric, 100::numeric * (r.minutes::numeric / (r.minutes + 380)::numeric)))::double precision) * (1::numeric - (1::numeric - r.wt) * 0.35)::double precision AS b
         ...
         SELECT r.card_id,
                CASE WHEN r.pos = 'GK'::text THEN GREATEST(0::double precision, LEAST(75::double precision, round((0.5::double precision * r.rel_pct * 100::double precision + (0.5 * LEAST(95::numeric, 100::numeric * (r.minutes::numeric / (r.minutes + 380)::numeric)))::double precision) * (1::numeric - (1::numeric - r.wt) * 0.35)::double precision)))
