 WITH proof_src AS (
         SELECT psc_p.id AS card_id,
            COALESCE(pp_p."position", psc_p."position") AS pool,
            (((psc_p.goals)::numeric * 90.0) / (NULLIF(psc_p.minutes, 0))::numeric) AS r_goals,
            (((psc_p.shots_on)::numeric * 90.0) / (NULLIF(psc_p.minutes, 0))::numeric) AS r_shots_on,
            (((psc_p.shots_total)::numeric * 90.0) / (NULLIF(psc_p.minutes, 0))::numeric) AS r_shots_total,
            (((psc_p.assists)::numeric * 90.0) / (NULLIF(psc_p.minutes, 0))::numeric) AS r_assists,
            (((psc_p.passes_key)::numeric * 90.0) / (NULLIF(psc_p.minutes, 0))::numeric) AS r_passes_key,
            (((psc_p.tackles_total)::numeric * 90.0) / (NULLIF(psc_p.minutes, 0))::numeric) AS r_tackles_total,
            (((psc_p.interceptions)::numeric * 90.0) / (NULLIF(psc_p.minutes, 0))::numeric) AS r_interceptions,
            (((psc_p.tackles_blocks)::numeric * 90.0) / (NULLIF(psc_p.minutes, 0))::numeric) AS r_tackles_blocks
           FROM ((player_season_cards psc_p
             LEFT JOIN players p_p ON ((psc_p.player_id = p_p.id)))
             LEFT JOIN player_positions pp_p ON (((pp_p.api_player_id = p_p.api_player_id) AND (pp_p.season_year = psc_p.season_year) AND (pp_p.league_code = psc_p.league_code))))
          WHERE (psc_p.minutes >= 300)
        ), proof_pct AS (
         SELECT proof_src.card_id,
                CASE
                    WHEN (proof_src.r_goals IS NOT NULL) THEN percent_rank() OVER (PARTITION BY proof_src.pool ORDER BY proof_src.r_goals)
                    ELSE NULL::double precision
                END AS pct_goals,
                CASE
                    WHEN (proof_src.r_shots_on IS NOT NULL) THEN percent_rank() OVER (PARTITION BY proof_src.pool ORDER BY proof_src.r_shots_on)
                    ELSE NULL::double precision
                END AS pct_shots_on,
                CASE
                    WHEN (proof_src.r_shots_total IS NOT NULL) THEN percent_rank() OVER (PARTITION BY proof_src.pool ORDER BY proof_src.r_shots_total)
                    ELSE NULL::double precision
                END AS pct_shots_total,
                CASE
                    WHEN (proof_src.r_assists IS NOT NULL) THEN percent_rank() OVER (PARTITION BY proof_src.pool ORDER BY proof_src.r_assists)
                    ELSE NULL::double precision
                END AS pct_assists,
                CASE
                    WHEN (proof_src.r_passes_key IS NOT NULL) THEN percent_rank() OVER (PARTITION BY proof_src.pool ORDER BY proof_src.r_passes_key)
                    ELSE NULL::double precision
                END AS pct_passes_key,
                CASE
                    WHEN (proof_src.r_tackles_total IS NOT NULL) THEN percent_rank() OVER (PARTITION BY proof_src.pool ORDER BY proof_src.r_tackles_total)
                    ELSE NULL::double precision
                END AS pct_tackles_total,
                CASE
                    WHEN (proof_src.r_interceptions IS NOT NULL) THEN percent_rank() OVER (PARTITION BY proof_src.pool ORDER BY proof_src.r_interceptions)
                    ELSE NULL::double precision
                END AS pct_interceptions,
                CASE
                    WHEN (proof_src.r_tackles_blocks IS NOT NULL) THEN percent_rank() OVER (PARTITION BY proof_src.pool ORDER BY proof_src.r_tackles_blocks)
                    ELSE NULL::double precision
         ...
         SELECT pool_pct_1.card_id,
            pool_pct_1.pool,
            pool_pct_1.def_share_pct AS sig
         ...
         SELECT psc3.id AS card_id,
          ...
          WHERE ((h.honour_type <> 'world_cup_winner'::text) AND (h.honour_type <> ALL (ARRAY['golden_boot'::text, 'top_assists'::text, 'player_of_season'::text, 'ballon_dor'::text])))
        UNION ALL
         SELECT perf.card_id,
            perf.honour_type,
            perf.h_year,
            perf.leg
           FROM ( SELECT DISTINCT ON (h.id, psc3.league_code, psc3.season) psc3.id AS card_id,
                    h.honour_type,
                    h.season_year AS h_year,
                    'player'::text AS leg
                   FROM ((player_season_cards psc3
                     JOIN players p3 ON ((psc3.player_id = p3.id)))
                     JOIN honours h ON (((h.api_player_id = p3.api_player_id) AND (h.season_year = psc3.season_year))))
                  WHERE (h.honour_type = ANY (ARRAY['golden_boot'::text, 'top_assists'::text, 'player_of_season'::text, 'ballon_dor'::text]))
                  ORDER BY h.id, psc3.league_code, psc3.season,
                    CASE h.honour_type
                        WHEN 'golden_boot'::text THEN COALESCE(psc3.goals, '-1'::integer)
                        WHEN 'top_assists'::text THEN COALESCE(psc3.assists, '-1'::integer)
                        ELSE COALESCE(psc3.minutes, '-1'::integer)
                    END DESC, COALESCE(psc3.minutes, '-1'::integer) DESC, COALESCE(psc3.appearances, '-1'::integer) DESC, psc3.team_id) perf
        UNION ALL
         SELECT psc3.id AS card_id,
            h.honour_type,
            h.season_year AS h_year,
        ...
        UNION ALL
         ...
         SELECT hon_rows.card_id,
            ...
            bool_or((hon_rows.honour_type = 'euro_winner'::text)) AS h_euro_winner,
            bool_or((hon_rows.honour_type = 'copa_winner'::text)) AS h_copa_winner
           FROM hon_rows
 ...
 SELECT psc.id AS card_id,
    ...
    proof_pct.pct_goals,
    proof_pct.pct_shots_on,
    proof_pct.pct_shots_total,
    proof_pct.pct_assists,
    proof_pct.pct_passes_key,
    proof_pct.pct_tackles_total,
    proof_pct.pct_interceptions,
    proof_pct.pct_tackles_blocks,
    COALESCE(hon.h_euro_winner, false) AS h_euro_winner,
    COALESCE(hon.h_copa_winner, false) AS h_copa_winner,
    psc.shirt_number_source
   FROM ((((((((((player_season_cards psc
     ...
     LEFT JOIN proof_pct ON ((proof_pct.card_id = psc.id)))
